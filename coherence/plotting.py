...
def coherence_bandwise(coh, plot_shuffled=False, plot_std=True, n_plots_per_page=6, methods=['coh', 'imcoh'],
    ...
    for method in methods: # plots data for different coherence calculations separately
        ...
        for reref_type in reref_types_idc.keys(): # plots data separately based on the type of rereferencing
            reref_type_idc = reref_types_idc[reref_type]

            # Gets a global y-axis for all data of the same type (if requested)
            if same_y == True:
                if plot_std == True and f'{method}_std' in coh.data_keys():
                    ylim = helpers.same_axes(coh[[key+'_std' for key in fullkeys_to_plot]].iloc[reref_type_idc])
                else:
                    ylim = helpers.same_axes(coh[fullkeys_to_plot].iloc[reref_type_idc])

            # Gets the indices of data to plot grouped based on the data's characteristics
            type_idcs = helpers.index_by_type(coh.iloc[reref_type_idc].copy(deep=True), dataset_info,
                                              avg_as_equal=True)

            for type_idc in type_idcs: # plots data separately based on the data's characteristics
                    ...
                    for row_i in range(n_rows): # fill up each row from top to down...
                        for col_i in range(n_cols): # ... and from left to right
                            if stop is False: # if there is still data to plot for this method
                                ...
                                type_idx = type_idc[ch_i]
                                data = coh.iloc[type_idx] # the data to plot

...
def psd_bandwise(psd, group_master, group_fig=[], group_plot=[],
    ...
    for power_key in power_keys:
        ...
        for key in keys_to_plot:
            fullkeys_to_plot.append(f'{power_key}_fbands_{key}')
                                ...
                                if len(data_keys) == 0: # if data of multiple conditions is not being plotted
                                    ...
                                    for band_i, band in enumerate(bands): # for each frequency band
                                        ...
                                        for key in fullkeys_to_plot:
                                            ...
                                            if power_key+'_fbands_max' in key: # gets the std of the fmax data to add to the plots
                                                fmaxs.append(str(int(data[power_key+'_fbands_fmax'][band_i])))
                                                if max_std_present == True:
                                                    fmax_std = u'\u00B1'+str(int(np.ceil(data[power_key+'_fbands_fmax_std'][band_i])))
                                                    fmaxs[-1] += fmax_std
                                    ...
                                    if 'max' in keys_to_plot:
                                        ...
                                        for fmax_i, fmax in enumerate(fmaxs):
                                            text_ypos = data[power_key+'_fbands_max'][fmax_i]+ylim[1]*.02 # where to put text
                                            if max_std_present == True:
                                                text_ypos += data[power_key+'_fbands_max_std'][fmax_i]
                                    ...
                                    for band_i, band in enumerate(bands): # for each frequency band
                                        for ch_i, ch_idx in enumerate(idc_group_plot): # for each piece of data in the group
                                            ...
                                            for key in fullkeys_to_plot:
                                                    ...
                                                    #... plots
                                                        fmaxs[ch_i].append(str(int(data.loc[ch_idx][power_key+'_fbands_fmax'][band_i])))
                                                        if max_std_present == True:
                                                            fmax_std = u'\u00B1'+str(int(
                                                                        np.ceil(data.loc[ch_idx][power_key+'_fbands_fmax_std'][band_i])))
                                    ...
                                    if 'max' in keys_to_plot:
                                        ...
                                        for fmax_i in range(len(fmaxs[0])):
                                            for ch_i in range(len(fmaxs)):
                                                ...
                                                sorted_ypos.append(data.iloc[ch_i][power_key+'_fbands_max'][fmax_i]+ylim[1]*.02)
                                                if max_std_present == True:
                                                    sorted_ypos[-1] += data.iloc[ch_i][power_key+'_fbands_max_std'][fmax_i]
...
def psd_bandwise_gb(psd, areas, group_master, group_fig=[], group_plot=[],
    ...
    for power_key in power_keys:
        for data_key_append in data_key_appends:
            power_data_keys.append(power_key+data_key_append)
    psd_data_keys = ['ch_coords', 'ch_coords_std', 'freqs', 'fbands', *power_keys, *power_data_keys]
            ...
            for mastergroup_i, idc_group_master in enumerate(idcs_group_master):
                ...
                for figgroup_i, idc_group_fig in enumerate(idcs_group_fig):
                    ...
                    for plot_key in fullkeys_to_plot:
                        ...
                        wind_title, included = helpers.window_title(fig_info, base_title=f'{plot_key}:',
                                                                    full_info=False)
